...
export default function GroceryBlog() {
    ...
    return (
        <section className="sp-blog padding-tb-50 aos-init" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="200">
            <div className="container">
                <div className="row">
                    <div className="section-detail detail-two">
                        <div className="sp-title">
                            <p><img src="/fonts/15.svg" alt="" />Recent Posts</p>
                            <h2 data-cursor="big" aria-label="Featured tips and tricks!" style={{ perspective: "500px" }}>
                                <div className="split-line" aria-hidden="true" style={{ position: "relative", display: "block", textAlign: "start" }}>
                                    <div aria-hidden="true" style={{ position: "relative", display: "inline-block" }}>Featured</div>{" "}
                                    <div aria-hidden="true" style={{ position: "relative", display: "inline-block" }}>tips</div>{" "}
                                    <div aria-hidden="true" style={{ position: "relative", display: "inline-block" }}>and</div>{" "}
                                    <div aria-hidden="true" style={{ position: "relative", display: "inline-block" }}>tricks!</div>
                                </div>
                            </h2>
                        </div>
                        <Link href="/blog" className="sp-btn-4">View all &nbsp;<i className="ri-arrow-right-long-line"></i></Link>
                    </div>
                </div>
                        ...
                        <div className="col-md-6 m-b-30" key={blog.id}>
                            <div className="sp-blog-box">
                                <div className="sp-blog-img">
                                    <img src={blog.image} alt="blog" style={{ translate: "none", rotate: "none", scale: "none", transform: "translate(0px, 0px)" }} />
                                </div>
                                <div className="sp-blog-detail">
                                    <div className="sp-blog-info">
                                        <p className="sp-post"><Link href="/blog">{blog.category}</Link>- {blog.date}</p>
                                        <h3><Link href="/blog-detail">{blog.title}</Link></h3>
                                        <p className="desc">{blog.desc}</p>
                                    </div>
                                    <Link href="/blog-detail" className="sp-read">Continue Reading <i className="ri-arrow-right-long-line"></i></Link>
                                </div>
                            </div>
                        </div>
                    ))}
